...
while failedAttempts<=maxFailedAttempts
    ...
    [A, I]=min( distanceCost_RRT(RRTree1(:,1:2),sample) ,[],1); % find closest as per the function in the metric node to the sample
    ...
    [A, I2]=min( distanceCost_RRT(RRTree2(:,1:2),newPoint) ,[],1); % find closest in the second tree
    if distanceCost_RRT(RRTree2(I2(1),1:2),newPoint)<disTh, % if both trees are connected
        pathFound=[newPoint I(1) I2(1)];extendFail=false;break; 
    end 
    [A, I3]=min( distanceCost_RRT(RRTree1(:,1:2),newPoint) ,[],1); % check if new node is not already pre-existing in the tree
    if distanceCost_RRT(newPoint,RRTree1(I3(1),1:2))<disTh, failedAttempts=failedAttempts+1;continue; end 
