...
wallpercent = 0;  % this percent of field is walls
...
while ~max(ismember(setOpen,goalposind)) && ~isempty(setOpen)%ismember 判断setOpen中的值是不是在goalposind中
 
%    test_c = (ismember(setOpen,goalposind))
%    test_b = max(ismember(setOpen,goalposind))
%    test_a = ~max(ismember(setOpen,goalposind))
    aa = size(setOpenCosts)
    bb = size(setOpenHeuristics)
...
%     field(ind2sub([n n],ceil(2))) = Inf;%ceil 里面填索引
...
% This function creates a pretty figure
    ...
    if isempty(gcbf)
      f1 = figure('Position',[600 100 500 500],'Units','Normalized', ...
        'MenuBar','none');
...
%     cmap = flipud(colormap('jet'));%控制扩展节点的颜色
