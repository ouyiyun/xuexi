...
source=[50 160]; % source position in Y, X format
goal=[490 490]; % goal position in Y, X format
stepsize=40; % size of each step of the RRT
disTh=35; % nodes closer than this threshold are taken as almost the same
...
[m, n]=size(map);
if display, imshow(map);rectangle('position',[1 1 n-1 m-1],'edgecolor','k'); end
...
imshow(map);
rectangle('position',[1 1 n-1 m-1],'edgecolor','k');
line(path(:,2),path(:,1));